<template>
  <div>
    ...
    <div v-if="animals.length > 0">
      <h2>
        <span v-if="name">{{name}}!</span> Here's your Zoo
      </h2>
      <ol>
        <animal v-for="item in animals" :animal="item" :icon="animalsCodes[item]" :description="animalsDescriptions[item]" :key="item"></animal>
      </ol>
    </div>
  </div>
</template>
